...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .overview-band.is-collapsed {{
      grid-template-columns: 1fr;
      padding-block: 0;
      background: #fcfcfa;
    }}

    .overview-band.is-collapsed .overview-card {{
      display: none;
    }}

    .overview-band.is-collapsed .overview-collapsed-summary {{
      display: flex;
    }}

    .overview-collapsed-summary {{
      display: none;
      align-items: center;
    ...
    .overview-collapsed-summary strong {{
      color: var(--ink);
...
<body>
  <header>
    ...
    <div class="header-actions">
      <button class="overview-toggle" id="overviewToggle" type="button" aria-expanded="true">Hide overview</button>
      <a class="index-link" id="indexLink" href="index.html">All maps</a>
    </div>
  </header>

  <section class="overview-band" id="overviewBand">
    <div class="overview-collapsed-summary" id="overviewCollapsedSummary">
      <strong id="overviewCollapsedTitle">Overview hidden</strong>
      <span id="overviewCollapsedMetrics"></span>
    </div>
  ...
  <script>
    ...
    const overviewCollapsedMetrics = document.getElementById("overviewCollapsedMetrics");
    ...
    const OVERVIEW_COLLAPSED_KEY = "internal-link-map-overview-collapsed-v1";
    ...
    let graphCaches = {{
      ...
      edgeKeySet: new Set(),
      normalizedNodeSearchText: new Map(),
      normalizedEdgeSourceText: new Map(),
      normalizedEdgeTargetText: new Map()
    }};
    ...
    function buildGraphCaches() {{
      ...
      const normalizedNodeSearchText = new Map();
      const normalizedEdgeSourceText = new Map();
      const normalizedEdgeTargetText = new Map();

      currentGraph.nodes.forEach(node => {{
        normalizedNodeSearchText.set(
          node.id,
          normalizeSearchText(`${{node.path || ""}} ${{node.label || ""}} ${{node.id || ""}}`)
        );
      }});
      ...
      currentGraph.edges.forEach(edge => {{
        ...
        if (!normalizedEdgeSourceText.has(edgeKey)) {{
          const sourceNode = nodesById.get(edge.source);
          normalizedEdgeSourceText.set(
            edgeKey,
            normalizeSearchText(`${{edge.source}} ${{sourceNode?.path || ""}} ${{sourceNode?.label || ""}}`)
          );
        }}
        if (!normalizedEdgeTargetText.has(edgeKey)) {{
          const targetNode = nodesById.get(edge.target);
          normalizedEdgeTargetText.set(
            edgeKey,
            normalizeSearchText(`${{edge.target}} ${{targetNode?.path || ""}} ${{targetNode?.label || ""}}`)
          );
        }}
      }});
      ...
      graphCaches = {{
        ...
        edgeKeySet,
        normalizedNodeSearchText,
        normalizedEdgeSourceText,
        normalizedEdgeTargetText
      }};
    }}

    function scheduleUpdateView(delay = 120) {{
    ...
    function setMetrics() {{
      ...
      overviewCollapsedMetrics.textContent = `${{formatNumber(viewNodes.length)}} pages and ${{formatNumber(viewEdges.length)}} links visible · ${{formatNumber(currentGraph.meta.uniquePages)}} pages in dataset`;
    }}

    function setOverviewCollapsed(collapsed) {{
      overviewBand.classList.toggle("is-collapsed", collapsed);
      overviewToggle.textContent = collapsed ? "Show overview" : "Hide overview";
      overviewToggle.setAttribute("aria-expanded", String(!collapsed));
      try {{
        window.localStorage.setItem(OVERVIEW_COLLAPSED_KEY, collapsed ? "true" : "false");
      }} catch {{
      }}
    ...
    function getStoredOverviewCollapsed() {{
      try {{
        return window.localStorage.getItem(OVERVIEW_COLLAPSED_KEY) === "true";
      }} catch {{
        return false;
      }}
    }}
    ...
    function renderMatchSummary(query) {{
      ...
      const contextKey = `${{focusedContext.rawQuery}}|||${{focusedContext.sourcePathQuery}}|||${{focusedContext.targetPathQuery}}|||${{directionFilter.value}}`;
      if (contextKey !== lastMatchSummaryContext) {{
        matchSummaryDismissed = false;
        lastMatchSummaryContext = contextKey;
      }}
      ...
      matchSummary.innerHTML = `<div class="match-summary-header"><div><h2>Focused link summary</h2><p>Visible links for <strong>${{escapeHtml(focusLabel)}}</strong>, based on the current direction and path filters.</p></div><button class="match-summary-close" id="matchSummaryClose" type="button" aria-label="Close focused link summary">&times;</button></div>${{body}}${{renderComparisonSection(focusedContext.rawQuery)}}`;
      document.getElementById("matchSummaryClose").addEventListener("click", () => {{
    ...
    function updateView() {{
      ...
      const queryParts = splitPathParts(rawQuery);
      const queryHasLocalePrefix = isLocaleSegment(queryParts[0]);
      const matchesSourcePath = edge => {{
        if (!sourcePathQuery) return true;
        const edgeKey = `${{edge.source}}|||${{edge.target}}`;
        return (graphCaches.normalizedEdgeSourceText.get(edgeKey) || "").includes(sourcePathQuery);
      }};
      const matchesTargetPath = edge => {{
        if (!targetPathQuery) return true;
        const edgeKey = `${{edge.source}}|||${{edge.target}}`;
        return (graphCaches.normalizedEdgeTargetText.get(edgeKey) || "").includes(targetPathQuery);
      }};
      let candidates;
      ...
      currentGraph.edges.forEach(edge => {{
        ...
        if (query && direction !== "all" && !((direction === "out" && matchedSearchIds.has(edge.source)) || (direction === "in" && matchedSearchIds.has(edge.target)))) return;
        if (!matchesSourcePath(edge) || !matchesTargetPath(edge)) return;
        if (sourceNoindex && String(edge.sourceNoindex) !== sourceNoindex) return;
        if (targetNoindex && String(edge.targetNoindex) !== targetNoindex) return;
        if (sourceStatusCode && String(edge.sourceStatusCode) !== sourceStatusCode) return;
        if (targetStatusCode && String(edge.targetStatusCode) !== targetStatusCode) return;
        if (edge.anchorSourceShare >= sitewideShare) hiddenCount += 1;
        if (edge.componentLike) componentCount += 1;
        if (shouldHideSitewide && edge.anchorSourceShare >= sitewideShare) return;
        if (shouldHideComponents && edge.componentLike) return;
        nextViewEdges.push(edge);
      }});
      viewEdges = nextViewEdges;
      ...
      function tick() {{
        ...
        for (let i = 0; i < viewNodes.length; i++) {{
          ...
          for (let j = i + 1; j < viewNodes.length; j++) {{
            ...
            const minSpacing = a.radius + b.radius + 10;
            const distance = Math.sqrt(dist2) || 1;
            const overlap = Math.max(0, minSpacing - distance);
            const force = (Math.min(1.8, 980 / dist2) + overlap * 0.05) * alpha;
            a.vx += dx * force;
            a.vy += dy * force;
            b.vx -= dx * force;
            b.vy -= dy * force;
        ...
        viewNodes.forEach(node => {{
          ...
          node.vx *= 0.82;
          node.vy *= 0.82;
          node.x = Math.max(node.radius + 14, Math.min(rect.width - node.radius - 14, node.x + node.vx));
          node.y = Math.max(node.radius + 14, Math.min(rect.height - node.radius - 14, node.y + node.vy));
        }});
    ...
    overviewToggle.addEventListener("click", () => {{
      setOverviewCollapsed(!overviewBand.classList.contains("is-collapsed"));
    }});
    ...
    setOverviewCollapsed(getStoredOverviewCollapsed());
    renderPreset();
